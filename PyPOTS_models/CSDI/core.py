    ...
    def forward(
        ...
        if calc_criterion:
            if self.training:  # for training
                ...
                side_info = self.get_side_info(observed_tp, cond_mask)
                validating_loss = self.backbone.calc_loss_valid(observed_data, cond_mask, torch.ones_like(indicating_mask), side_info)
            ...
            results["imputation"] = imputed_data.permute(
                0, 1, 3, 2

...
class ImputeFormer(BaseNNImputer):
    ...
    def __init__(
        ...
        n_features = args.var_num
        n_layers = args.enc_num
        d_input_embed = args.model_dim
        d_learnable_embed = args.model_dim
        d_proj = args.model_dim
        d_ffn = args.model_dim * args.heads_num
        n_temporal_heads = args.heads_num
        ...
        if args.training_loss.lower() == 'mae':
            training_loss = MAE
        else:
            training_loss = MSE
            
        if args.validation_metric.lower() == 'mae':
    ...
    def fit(
        ...
        train_dataset = DatasetForSAITS(train_set, return_X_ori=True, return_y=False, file_type=file_type)
    ...
    def impute(
        ...
        **kwargs,
        ...
        """Impute the missing values for the given test set.

        Parameters
        ----------
        test_set :
            The test set for imputation. It can be a dictionary or a string path to the dataset file.
        file_type :
            The file type of the dataset if test_set is given as a string path. Default is "hdf5".
        
        Returns
        -------
        dict :
            A dictionary containing the imputed results.
        """
        ...
        results = result_dict["reconstruction"]
        return results